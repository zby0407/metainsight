...
const WatchlistRowItem: React.FC<{
  ...
  const researchPath = `/workspace/chats/new?stock=${encodeURIComponent(row.code)}&name=${encodeURIComponent(stockName)}`;
  const researchHref = `/login?next=${encodeURIComponent(researchPath)}`;
  ...
  return (
    <div className="home-subpanel grid min-w-0 gap-2 px-3 py-2.5">
      ...
      <a
        href={researchHref}
        className="flex h-8 items-center justify-center gap-1.5 rounded-lg border border-primary/25 bg-primary/10 px-2 text-xs font-medium text-primary transition-colors hover:bg-primary/15"
        title={`将 ${stockName} 加入 Agent 研究上下文`}
      >
