...
class PortfolioSandboxService:
    ...
    def scenario(
        ...
        *,
        ...
        symbols = sorted({symbol for symbol, weight in final_weights.items() if weight > WEIGHT_EPS})
        ...
        builder = EvidencePackBuilder(pack_type="sandbox", account_id=account_id, as_of=end_date)
        builder.add_input(
            "stock_daily",
            f"历史收盘价（{start_date.isoformat()} ~ {end_date.isoformat()}，缓存）",
        ...
        if not usable:
            builder.add_gap("critical", "窗口内所有标的均无历史行情，无法回放")
            return {
                "pack": builder.build(),
                "data": {
                    ...
                    "proposed_return_pct": None,
                    "baseline_max_drawdown_pct": None,
                    "proposed_max_drawdown_pct": None,
                    "series": [],
                    "weights_used": {symbol: round(weight, 6) for symbol, weight in final_weights.items()},
                },
            }

        common_dates = sorted(set.intersection(*[set(closes) for closes in usable.values()]))
        if len(common_dates) < 2:
            builder.add_gap("critical", "共同交易日不足两天，无法回放")
            common_dates = []
        ...
        fact_baseline = builder.add_fact(
            "基线组合历史收益",
            round(baseline_return, 4) if baseline_return is not None else None,
            unit="%",
            precision=4,
        )
        fact_proposed = builder.add_fact(
            "实验组合历史收益",
            round(proposed_return, 4) if proposed_return is not None else None,
            unit="%",
            precision=4,
        )
        if baseline_return is not None and proposed_return is not None:
            builder.add_fact(
                "实验相对基线差异",
                round(proposed_return - baseline_return, 4),
                unit="%",
                precision=4,
                source_fact_ids=[fact_baseline, fact_proposed],
            )
        builder.add_fact("基线最大回撤", round(baseline_dd, 4) if baseline_dd is not None else None, unit="%", precision=4)
        builder.add_fact("实验最大回撤", round(proposed_dd, 4) if proposed_dd is not None else None, unit="%", precision=4)
        builder.add_gap(
            "info",
            "历史确定性投影不代表未来收益；未包含交易费用、滑点与税费",
        )
        ...
        return {
            "pack": builder.build(),
            "data": {
                "start_date": common_dates[0].isoformat() if common_dates else start_date.isoformat(),
                "end_date": common_dates[-1].isoformat() if common_dates else end_date.isoformat(),
