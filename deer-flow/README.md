...
### MetaInsight News Workspace

MetaInsight includes an authenticated news workspace that groups reports from
multiple publishers into one event, keeps every source article available for
verification and follow-up questions, and separates corroborated claims from
single-source statements. Ranking freshness is calculated at read time; saved
events, followed topics, and notification cursors are account-scoped. Browsers
access the shared aggregator only through the Gateway's read-only
`/api/v1/news/*` facade rather than a public upstream service route.
The personalized landing feed de-duplicates and ranks all three channels, then
caps the initial result at 20 events so source breadth does not turn into an
unbounded page. It presents one unified ranked stream instead of separate modes
whose results are operationally indistinguishable. Topic preferences and alerts
live behind the header's Follow button; the desktop side rail stays
complementary with local weather and market context. Weather uses granted
browser location at city-level precision and falls back to Shanghai without
forcing a location prompt.

Production frontend builds cache the pnpm store and accept `NPM_REGISTRY`
(the bundled release script defaults to the mainland mirror), so a transient
registry reset does not restart the full dependency download.

### Skills & Tools

DeerFlow can also register server-owned business capability bridges as native
Agent tools. The bundled `portfolio-deep-research` skill coordinates personal
portfolio strategy, daily review, history attribution, and isolated simulation
work through a live capability catalog. Authenticated user/task identity is
injected at runtime, and confirm-level mutations require a separate exact user
reply before execution. Configure the three `finance-agent` tools shown in
`config.example.yaml` and provide `DEERFLOW_FINANCE_BRIDGE_SECRET` to the Gateway.
The native investment workspace can also display existing portfolios directly
through the authenticated, read-only `/api/finance/portfolio-dashboard`
projection. This projection is owner-scoped and no-store; portfolio research,
simulations, and every mutation still run through the Agent tools.
Docker deployments automatically attach the Gateway (not the frontend) to the
external `pipecat_default` network when that secret is configured; override the
network name with `DEERFLOW_FINANCE_NETWORK` when needed.
...
- **Do not expose internal data services**: Browser traffic must pass through the authenticated Gateway. Keep internal finance/market workers on the private container network and block legacy direct proxy paths such as `/market-api/`.
