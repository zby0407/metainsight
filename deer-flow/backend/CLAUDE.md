...
MetaInsight's `app/gateway/dsa_automation.py` owns account-scoped post-close DSA
scheduling and the persistent notification inbox. The Gateway lifespan must
start `run_dsa_automation_loop()`, and `create_app()` must mount both the
`/api/v1/dsa-automation` and `/api/v1/notifications` routers. Its post-close
readiness gate prefers a same-session daily bar, then uses a
cross-checked quote fallback after
`DSA_AUTOMATION_QUOTE_FALLBACK_MINUTES` (default 5). The fallback must have a
complete, internally consistent OHLCV snapshot, its `prev_close` must match the
latest daily close, and its OHLCV must differ from the latest daily bar. Keep
these invariants covered by `backend/tests/test_dsa_automation.py`; they
prevent both delayed-bar missed runs and stale holiday-quote false positives.

`app/gateway/routers/market_proxy.py` is the only browser-facing path to the
shared stock server. Its allowlist treats quote/K-line/intelligence reads as
public market data and fails closed for every unknown upstream route.
Watchlists live in `zhiheng_dsa_tenant_watchlist`; task/query ownership lives
in `zhiheng_dsa_tenant_tasks`; history and decision signals are released only
when their `query_id`/`trace_id` is owned by the current user. Migration
`0005_dsa_tenant_isolation` creates these projections and imports scheduler
task ownership. Keep `/market-api/` blocked at the outer reverse proxy.

`app/gateway/routers/news_proxy.py` is the only browser-facing path to the
shared news aggregator. It allows authenticated `GET`/`HEAD` reads for feed,
rankings, health, and UUID event detail only; unknown paths and all mutations
fail closed. Account-scoped topics, saved event IDs, notification enablement,
and durable notification cursors live in `zhiheng_news_preferences` through
`news_preferences.py`. Notification-center reads may materialize matching news
updates, but upstream polling is claimed per user for two minutes so browser
polling cannot fan out into repeated three-channel requests. Keep the public
`/finance-api/` prefix blocked at the outer reverse proxy.
...
| **Skills** (`/api/skills`) | `GET /` - list global skills; `GET /{name}` - details; admin-only `PUT /{name}`, custom edit/delete/rollback, and `POST /install` mutations (accepts standard optional frontmatter like `version`, `author`, `compatibility`) |
...
Business capability bridges should follow the
`deerflow.community.finance_agent` pattern: keep domain schemas and mutation
logic in the owning service, inject authenticated `user_id`, `thread_id`, and
`run_id` from `ToolRuntime`, expose no identity fields to the model, and bind
`auto` actions to durable intents that are prepared and executed in one tool
call. Reserve `confirm` for future destructive or external actions and bind
those to an exact subsequent `HumanMessage`. The bundled
portfolio bridge uses `DEERFLOW_FINANCE_BRIDGE_SECRET` plus the optional
`DEERFLOW_FINANCE_BRIDGE_URL` environment variable and is orchestrated by the
`portfolio-deep-research` skill.
The Gateway's `/api/finance/portfolio-dashboard` route is the one exception for
direct UI reads: it is an authenticated, owner-scoped, no-store projection for
displaying existing portfolio state. Keep it read-only; all research and writes
must continue through the Agent capability catalog. The page may hand compact
intent text and exact resource IDs to the Agent, but must not encode tool
selection or workflow policy in prompts.
In Docker production, `scripts/deploy.sh` conditionally adds
`docker/docker-compose.finance.yaml`; keep the finance service network external
and attach only the Gateway.
...
- User profiles are stored at `{base_dir}/users/{user_id}/USER.md`
- Per-agent per-user memory at `{base_dir}/users/{user_id}/agents/{agent_name}/memory.json`
- Custom agent definitions (`SOUL.md` + `config.yaml`) are also per-user at `{base_dir}/users/{user_id}/agents/{agent_name}/`. The legacy shared layout is hidden by default; `DEER_FLOW_ALLOW_LEGACY_SHARED_AGENTS=true` is a migration-window-only compatibility switch
...
- **Migration**: Run `PYTHONPATH=. python scripts/migrate_user_isolation.py` to move legacy `memory.json`, `USER.md`, `threads/`, and `agents/` into per-user layout. Supports `--dry-run` (preview changes) and `--user-id USER_ID` (assign unowned legacy data to a user, defaults to `default`).
...
- `migrations/versions/0003_user_admin_console.py` — account administration policy fields
- `migrations/versions/0004_dsa_auto_research.py` — per-user DSA schedule and notification inbox
- `migrations/versions/0005_dsa_tenant_isolation.py` — private watchlist and upstream research ownership projection
- `migrations/versions/0006_news_preferences.py` — account-scoped news topics, saved events, and durable notification polling cursors
