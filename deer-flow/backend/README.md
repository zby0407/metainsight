...
The `deerflow.community.finance_agent` provider is a native, identity-aware
bridge to a separately deployed portfolio capability service. It exposes a
live catalog and generic schema-validated execution without placing portfolio
business rules in DeerFlow prompts or frontend code. Local recorded-state and
simulation actions use `actionLevel=auto`: they create and resolve an audited,
thread-bound intent in one tool call. `actionLevel=confirm` remains available
for future destructive or external actions. The provider reads
`DEERFLOW_FINANCE_BRIDGE_SECRET` and optional
`DEERFLOW_FINANCE_BRIDGE_URL` from the Gateway environment; user, thread, and
run IDs come only from `ToolRuntime`.
The authenticated Gateway route `/api/finance/portfolio-dashboard` uses the
same server-side identity bridge to expose a read-only projection of the current
user's existing portfolios to the native investment workspace. It generates
request-local thread/run IDs, disables browser caching, and never forwards the
bridge secret to the frontend. Mutations and research execution remain Agent
tool responsibilities; dashboard quick actions pass only a compact user intent
and selected portfolio ID back into the Agent.
The optional `docker-compose.finance.yaml` overlay is enabled by `deploy.sh`
when the bridge secret is configured, limiting finance-network access to the
Gateway container.
...
The MetaInsight Gateway also runs account-scoped post-close DSA automation. It
checks for the current session's daily bar after 15:10
Asia/Shanghai and, after a configurable grace period, may accept a complete
realtime OHLCV snapshot only when its previous close matches the latest daily
bar and the snapshot differs from that bar. This keeps delayed daily-bar feeds
from suppressing a run without treating a stale holiday quote as a new session.
Completed and failed runs are written to the persistent notification inbox at
`/api/v1/notifications`; the frontend reuses the standard browser notification
permission and preference for background desktop delivery.

The stock service is reached through the authenticated `/api/v1/market/*`
facade. Quotes, price history, and public intelligence remain shared market
data; watchlists, analysis tasks, research history, and decision signals are
projected through Gateway ownership tables and filtered by the current user.
The upstream `/market-api/*` route must not be exposed publicly. Pre-isolation
DSA state is imported once for the oldest administrator.
...
| `GET/PUT /api/v1/dsa-automation/settings` | Configure the current account's post-close DSA symbols and schedule |
| `GET /api/v1/notifications` | List the current account's persistent research notifications |
| `POST /api/v1/notifications/{id}/read` | Mark one research notification as read |
| `POST /api/v1/notifications/read-all` | Mark all research notifications as read |
| `/api/v1/market/*` | Authenticated tenant facade for public market data and account-private DSA state |
...
Stdio MCP servers use `session_scope: "thread"` by default, preserving per-user
conversation state and filesystem isolation. A stateless, multi-user-safe API
adapter can explicitly use `session_scope: "global"`; the Gateway then starts
and discovers it during startup and reuses the warm process across conversations.
