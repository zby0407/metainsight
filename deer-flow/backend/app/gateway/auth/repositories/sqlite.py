...
from deerflow.persistence.user.model import AuthSettingRow, UserRow
...
class SQLiteUserRepository(UserRepository):
    ...
    async def count_active_admin_users(self) -> int:
        stmt = (
            select(func.count())
            .select_from(UserRow)
            .where(UserRow.system_role == "admin", UserRow.is_active.is_(True))
        )
        async with self._sf() as session:
            return await session.scalar(stmt) or 0
    ...
    async def get_setting(self, key: str) -> str | None:
        async with self._sf() as session:
            row = await session.get(AuthSettingRow, key)
    ...
    async def set_setting(self, key: str, value: str) -> None:
        async with self._sf() as session:
            row = await session.get(AuthSettingRow, key)
            if row is None:
                session.add(AuthSettingRow(key=key, value=value))
            else:
                row.value = value
            await session.commit()
