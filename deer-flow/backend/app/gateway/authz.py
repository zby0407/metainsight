...
def require_permission(
    ...
    Args:
        ...
        require_existing: Only meaningful with ``owner_check=True``. The secure
                          default requires a persisted, owned ``threads_meta``
                          row. Set False only for a creation path that is allowed
                          to establish a new thread-scoped resource.

    Usage:
        # Read-style: the thread must exist and be owned
        @require_permission("threads", "read", owner_check=True)
        async def get_thread(thread_id: str, request: Request):
            ...
    ...
    def decorator(func: Callable[P, T]) -> Callable[P, T]:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            # ``ThreadMetaStore.check_access``. Missing and NULL-owner rows
            # are denied unless the route explicitly opts into missing-row
            # creation semantics.
            if owner_check:
