...
from deerflow.config.agents_config import AgentConfig, legacy_shared_agents_enabled, list_custom_agents, load_agent_config, load_agent_soul
...
async def check_agent_name(name: str) -> dict:
    ...
    user_id = get_effective_user_id()
    paths = get_paths()
    available = not paths.user_agent_dir(user_id, normalized).exists()
    if legacy_shared_agents_enabled():
        available = available and not paths.agent_dir(normalized).exists()
...
async def create_agent_endpoint(request: AgentCreateRequest) -> AgentResponse:
    ...
    def _create_agent() -> AgentResponse | None:
        ...
        if legacy_shared_agents_enabled() and legacy_dir.exists():
...
async def update_agent(name: str, request: AgentUpdateRequest) -> AgentResponse:
    ...
    if legacy_shared_agents_enabled() and not agent_dir.exists() and paths.agent_dir(name).exists():
        raise HTTPException(
            status_code=409,
...
class UserProfileResponse(BaseModel):
    """Response model for the current user's profile (USER.md)."""

    content: str | None = Field(default=None, description="USER.md content, or null if not yet created")


class UserProfileUpdateRequest(BaseModel):
    """Request body for setting the current user's profile."""
...
@router.get(
    ...
    description="Read the current user's private USER.md profile.",
)
...
@router.put(
    ...
    description="Write the current user's private USER.md profile.",
)
...
async def delete_agent(name: str) -> None:
    ...
    def _remove_agent_dir() -> tuple[str, str]:
        ...
        if not agent_dir.exists():
            outcome = "legacy" if legacy_shared_agents_enabled() and paths.agent_dir(name).exists() else "missing"
