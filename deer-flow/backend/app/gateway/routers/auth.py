...
async def register(request: Request, response: Response, body: RegisterRequest):
    ...
    return UserResponse(id=str(user.id), email=user.email, system_role=user.system_role, is_active=user.is_active)
...
async def initialize_admin(request: Request, response: Response, body: InitializeAdminRequest):
    ...
    # First-boot initialization happens after the lifespan's initial
    # no-admin check. Re-run the idempotent legacy ownership migration now so
    # NULL-owner SQL rows and pre-auth LangGraph threads are never exposed as
    # shared or left inaccessible until a later restart.
    try:
        from app.gateway.app import _ensure_admin_user

        await _ensure_admin_user(request.app)
    except Exception:
        logger.exception("Post-initialization legacy ownership migration failed")
