...
async def _require_skill_admin(request: Request) -> None:
    await require_admin_user(
        request,
        detail="Administrator access is required to modify global skills",
    )
...
async def install_skill(request: SkillInstallRequest, http_request: Request, config: AppConfig = Depends(get_config)) -> SkillInstallResponse:
    await _require_skill_admin(http_request)
    try:
...
async def delete_custom_skill(skill_name: str, request: Request, config: AppConfig = Depends(get_config)) -> dict[str, bool]:
    await _require_skill_admin(request)
    try:
        skill_name = skill_name.replace("\r\n", "").replace("\n", "")
        storage = get_or_new_skill_storage(app_config=config)
