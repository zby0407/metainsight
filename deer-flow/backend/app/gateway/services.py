...
async def start_run(
    ...
    request: Request,
    *,
    allow_missing_thread: bool = False,
) -> RunRecord:
    ...
    # checkpoint state from) another user's thread. Only a stateless request
    # for which the server generated a fresh id may create a missing row;
    # supplied ids and legacy NULL-owner rows are denied. Internal
    ...
    # thread access.
    user = getattr(request.state, "user", None)
    effective_owner_user_id = owner_user_id or (
    ...
    if user is not None:
        allowed = await run_ctx.thread_store.check_access(
            thread_id,
            str(user.id),
            require_existing=not allow_missing_thread,
        )
        ...
        try:
            ...
            if existing is None and effective_owner_user_id:
                ...
                if unscoped_existing is not None:
                    if unscoped_existing.get("user_id") != effective_owner_user_id:
