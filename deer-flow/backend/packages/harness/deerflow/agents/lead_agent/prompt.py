...
[citation:LangGraph Documentation](https://langchain-ai.github.io/langgraph/). It supports
durable execution, streaming, and human-in-the-loop workflows
[citation:LangGraph Overview](https://langchain-ai.github.io/langgraph/concepts/why-langgraph/).
...
- ✅ RIGHT in Sources: `[LangGraph Documentation](https://langchain-ai.github.io/langgraph/) - 官方文档`
...
def _get_memory_context(agent_name: str | None = None, *, app_config: AppConfig | None = None) -> str:
    ...
    try:
        ...
        extra = ""
        if "[investor-risk-profile]" in memory_content:
            extra = (
                "\nWhen proposing or executing any portfolio rebalancing, position-size change, "
                "strategy update, or sandbox rollout, honor the investor risk profile in this "
                "memory (cash floor and single-stock cap) and cite it in the change basis."
            )

        return f"""<memory>
{memory_content}{extra}
</memory>
"""
    except Exception:
...
def apply_prompt_template(
    ...
    *,
    ...
    return SYSTEM_PROMPT_TEMPLATE.format(
        agent_name=agent_name or "MetaInsight",
