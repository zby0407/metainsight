...
class TitleMiddleware(AgentMiddleware[TitleMiddlewareState]):
    ...
    def _explicit_message_title(self, state: TitleMiddlewareState) -> str | None:
        """Return a bounded UI title attached to a structured workflow message."""
        config = self._get_title_config()
        for message in state.get("messages", []):
            if not self._is_user_message_for_title(message):
                continue
            additional_kwargs = getattr(message, "additional_kwargs", None)
            if not isinstance(additional_kwargs, dict):
                continue
            workspace = additional_kwargs.get("investment_workspace")
            if not isinstance(workspace, dict):
                continue
            display_label = workspace.get("display_label")
            if not isinstance(display_label, str):
                continue
            title = re.sub(r"\s+", " ", display_label).strip()
            if title:
    ...
    def _generate_title_result(self, state: TitleMiddlewareState) -> dict | None:
        ...
        explicit_title = self._explicit_message_title(state)
        if explicit_title:
            return {"title": explicit_title}
    ...
    async def _agenerate_title_result(self, state: TitleMiddlewareState) -> dict | None:
        ...
        # Title generation runs on the critical path after the first model
        # response. For latency-sensitive deployments, keep useful automatic
        # titles without paying for a second model round trip.
        if config.generation_mode == "local":
            return {"title": self._fallback_title(user_msg)}
