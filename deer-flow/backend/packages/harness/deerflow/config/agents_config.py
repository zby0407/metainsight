"""Configuration and strictly tenant-scoped loaders for custom agents."""
...
def legacy_shared_agents_enabled() -> bool:
    """Return whether the temporary pre-isolation read fallback is enabled."""
    return os.getenv("DEER_FLOW_ALLOW_LEGACY_SHARED_AGENTS", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def resolve_agent_dir(name: str, *, user_id: str | None = None) -> Path:
    ...
    Resolution uses the current user's directory. A temporary legacy fallback
    exists only when ``DEER_FLOW_ALLOW_LEGACY_SHARED_AGENTS=true`` is explicitly
    set during a controlled migration window.
...
def load_agent_config(name: str | None, *, user_id: str | None = None) -> AgentConfig | None:
    ...
    Reads from the per-user layout. Legacy shared reads require an explicit
    migration-only environment flag.
...
def list_custom_agents(*, user_id: str | None = None) -> list[AgentConfig]:
    ...
    Returns agents belonging to the effective user. Legacy shared entries are
    included only when the migration-only fallback flag is enabled.
