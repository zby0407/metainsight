...
class TitleConfig(BaseModel):
    ...
    generation_mode: Literal["model", "local"] = Field(
        default="model",
        description=("How to generate the first thread title. 'model' uses an extra LLM call; 'local' derives it from the first user message with no added latency."),
    )
    max_words: int = Field(
        default=6,
        ge=1,
