...
def _make_session_pool_tool(
    ...
    session_scope: Literal["thread", "global"] = "thread",
) -> BaseTool:
    """Wrap an MCP tool so it reuses a persistent session from the pool.

    Replaces the per-call session creation with pool-managed sessions. The
    default ``thread`` scope uses ``(server_name, user_id:thread_id)`` so
    stateful MCP servers (e.g. Playwright) remain isolated per conversation.
    Explicitly stateless, multi-user-safe servers may opt into ``global`` scope
    to reuse one warm subprocess across conversations.
    ...
    async def call_with_persistent_session(
        ...
        **arguments: Any,
        ...
        # Thread scope is the secure default. Global scope is an explicit
        # operator opt-in for stateless API adapters and similar servers.
        scope_key = "global" if session_scope == "global" else f"{user_id}:{thread_id}"
        ...
        if is_stdio and session_scope == "thread":
            paths = get_paths()
        ...
        return await asyncio.to_thread(
            ...
            call_tool_result,
            # Global subprocesses are deliberately not tied to any user's
            # filesystem. Do not translate their local paths into a thread's
            # private virtual mount.
            thread_id=thread_id if session_scope == "thread" else None,
            user_id=user_id if session_scope == "thread" else None,
...
async def get_mcp_tools() -> list[BaseTool]:
    ...
    try:
        ...
        # Stateless stdio servers configured with global session scope are
        # discovered through the same persistent session used for tool calls.
        # This avoids launching one process for discovery and another on the
        # first request. All other servers retain the adapter's standard
        # temporary-session discovery behavior.
        enabled_servers = extensions_config.get_enabled_mcp_servers()
        global_stdio_servers = {name for name, server in enabled_servers.items() if name in servers_config and server.type == "stdio" and server.session_scope == "global"}
        regular_servers_config = {name: connection for name, connection in servers_config.items() if name not in global_stdio_servers}

        tools: list[BaseTool] = []
        if regular_servers_config:
            client = MultiServerMCPClient(
                regular_servers_config,
                tool_interceptors=tool_interceptors,
                tool_name_prefix=True,
            )
            tools.extend(await client.get_tools())

        if global_stdio_servers:
            from langchain_mcp_adapters.tools import load_mcp_tools

            pool = get_session_pool()
            for server_name in global_stdio_servers:
                session = await pool.get_session(server_name, "global", servers_config[server_name])
                tools.extend(
        ...
        for tool in tools:
            ...
            if tool_server is not None:
                ...
                if transport == "stdio":
                    server_config = enabled_servers[tool_server]
                    wrapped_tools.append(
                        _make_session_pool_tool(
                            ...
                            tool_interceptors,
                            session_scope=server_config.session_scope,
                        )
                    )
                else:
