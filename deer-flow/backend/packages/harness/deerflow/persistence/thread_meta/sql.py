...
class ThreadMetaRepository(ThreadMetaStore):
    ...
    async def check_access(self, thread_id: str, user_id: str, *, require_existing: bool = True) -> bool:
        """Check if ``user_id`` has access to ``thread_id``.

        NULL-owner rows are never treated as shared. Startup migration assigns
        legacy rows to the first administrator before authenticated traffic is
        accepted. ``require_existing=False`` is reserved for creating a new
        thread-scoped resource such as an upload directory.
        """
        async with self._sf() as session:
            row = await session.get(ThreadMetaRow, thread_id)
            if row is None:
