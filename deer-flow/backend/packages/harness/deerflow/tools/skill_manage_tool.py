...
async def _skill_manage_impl(
    ...
    """
    runtime_context = runtime.context if runtime is not None and isinstance(runtime.context, dict) else {}
    if runtime_context.get("user_id") and runtime_context.get("system_role") != "admin":
        raise PermissionError("Only an administrator may modify global skills.")
