"""One-time migration: move legacy files into a per-user layout.
...
def migrate_user_profile(
    ...
    *,
    ...
    """Move the legacy global USER.md to the selected user's profile."""
    legacy_profile = paths.user_md_file
    if not legacy_profile.exists():
        logger.info("No legacy USER.md found — nothing to migrate.")
        return

    destination = paths.user_profile_file(user_id)
    if destination.exists():
        backup = paths.base_dir / "USER.legacy.md"
        logger.warning("Destination %s exists; renaming legacy to %s", destination, backup)
        if not dry_run:
            legacy_profile.rename(backup)
        return

    logger.info("Migrating USER.md -> %s", destination)
    if not dry_run:
...
def _build_owner_map_from_db(paths: Paths) -> dict[str, str]:
    ...
    # Persistence stores the application database below ``data/``.  The old
    # pre-persistence filename (``deer-flow.db``) was never the production DB
    # and caused this migration to silently build an empty ownership map.
