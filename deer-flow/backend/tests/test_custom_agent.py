...
def _legacy_agent_compatibility_for_legacy_tests(monkeypatch):
    """Legacy loader tests opt into the migration-only compatibility flag."""
    monkeypatch.setenv("DEER_FLOW_ALLOW_LEGACY_SHARED_AGENTS", "true")
...
class TestPaths:
    ...
    def test_user_profile_file_is_scoped(self, tmp_path):
        paths = _make_paths(tmp_path)
        assert paths.user_profile_file("user-a") == tmp_path / "users" / "user-a" / "USER.md"
        assert paths.user_profile_file("user-a") != paths.user_profile_file("user-b")
...
class TestResolveAgentDirMemoryOnlyFallback:
    ...
    def test_legacy_agent_is_hidden_without_explicit_flag(self, tmp_path, monkeypatch):
        from deerflow.config.agents_config import resolve_agent_dir

        legacy_dir = tmp_path / "agents" / "my-agent"
        legacy_dir.mkdir(parents=True)
        (legacy_dir / "config.yaml").write_text("name: my-agent\n", encoding="utf-8")
        monkeypatch.delenv("DEER_FLOW_ALLOW_LEGACY_SHARED_AGENTS", raising=False)

        with patch("deerflow.config.agents_config.get_paths", return_value=_make_paths(tmp_path)):
            result = resolve_agent_dir("my-agent", user_id="u1")

        assert result == tmp_path / "users" / "u1" / "agents" / "my-agent"
