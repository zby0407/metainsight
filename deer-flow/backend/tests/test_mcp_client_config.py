...
def test_mcp_server_session_scope_is_thread_isolated_by_default():
    config = McpServerConfig(type="stdio", command="npx")

    assert config.session_scope == "thread"


def test_mcp_server_session_scope_accepts_explicit_global_reuse():
    config = McpServerConfig(type="stdio", command="npx", session_scope="global")

    assert config.session_scope == "global"


def test_mcp_server_session_scope_rejects_unknown_value():
    with pytest.raises(ValueError, match="session_scope"):
        McpServerConfig.model_validate({"type": "stdio", "command": "npx", "session_scope": "process"})
