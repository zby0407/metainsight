...
@pytest.mark.asyncio
async def test_session_pool_tool_global_scope_reuses_one_process_without_user_workspace():
    """Stateless global servers reuse one process across conversation IDs."""
    from langchain_core.tools import StructuredTool
    from pydantic import BaseModel, Field
    ...
    original_tool = StructuredTool(
        name="daily_stock_analysis_quote",
    ...
    with (
        ...
        patch(
            ...
            side_effect=AssertionError("global scope must not create a user workspace"),
        ),
    ):
        ...
        for thread_id in ("thread-a", "thread-b"):
            ...
            runtime.config = {}
            await wrapped.coroutine(runtime=runtime, symbol="DSA")

    assert create_session.call_count == 1
    assert ("daily_stock_analysis", "global") in get_session_pool()._entries
...
@pytest.mark.asyncio
async def test_global_stdio_discovery_reuses_the_pooled_session():
    """Global stdio discovery and later tool calls share the same warm session."""
    from langchain_core.tools import StructuredTool
    from pydantic import BaseModel, Field
    ...
    extensions_config = MagicMock()
    extensions_config.get_enabled_mcp_servers.return_value = {"daily_stock_analysis": server_config}
    extensions_config.model_extra = {}
    servers_config = {
        "daily_stock_analysis": {
            "transport": "stdio",
    ...
    with (
        ...
        patch("deerflow.mcp.tools.get_initial_oauth_headers", return_value={}),
        patch("deerflow.mcp.tools.build_oauth_tool_interceptor", return_value=None),
        patch("langchain_mcp_adapters.client.MultiServerMCPClient") as multi_client,
        patch("langchain_mcp_adapters.sessions.create_session", return_value=mock_cm) as create_session,
        patch(
        ...
        tools = await get_mcp_tools()
        await tools[0].coroutine(symbol="DSA")

    multi_client.assert_not_called()
    create_session.assert_called_once()
    load_tools.assert_awaited_once_with(
        mock_session,
        tool_interceptors=[],
        server_name="daily_stock_analysis",
        tool_name_prefix=True,
    )
    assert ("daily_stock_analysis", "global") in get_session_pool()._entries
    mock_session.call_tool.assert_awaited_once_with("quote", {"symbol": "DSA"})
