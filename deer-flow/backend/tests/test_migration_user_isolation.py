...
class TestMigrateThreadDirs:
    def test_reads_thread_owners_from_runtime_database(self, paths: Paths):
        database = paths.base_dir / "data" / "deerflow.db"
        ...
        assert _build_owner_map_from_db(paths) == {"thread-a": "user-a"}
...
class TestMigrateUserProfile:
    def test_moves_global_profile_to_selected_user(self, paths: Paths):
        paths.user_md_file.write_text("private profile", encoding="utf-8")

        from scripts.migrate_user_isolation import migrate_user_profile

        migrate_user_profile(paths, user_id="admin-1")

        assert paths.user_profile_file("admin-1").read_text(encoding="utf-8") == "private profile"
