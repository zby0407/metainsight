...
``check_access`` semantics under test — missing, unowned, and foreign-owner
rows all deny — are exercised through real code.
"""
...
# ---------------------------------------------------------------------------
# Allowed: owner, auto-created stateless threads, scoped internal role
# ---------------------------------------------------------------------------
...
def test_stream_without_thread_id_passes_owner_check():
    ...
    assert response.status_code == 409
    create_or_reject.assert_awaited()
    assert create_or_reject.await_args.kwargs["user_id"] == str(USER_B.id)


def test_stream_untracked_thread_returns_404():
    """A supplied legacy thread id without ownership metadata is denied."""
    with _client(USER_B) as (client, create_or_reject):
...
def test_stream_shared_thread_returns_404():
    """A NULL-owner pre-auth row must be migrated before it can be read."""
    with _client(USER_B) as (client, create_or_reject):
...
def test_stream_internal_role_without_owner_header_is_scoped_to_internal_user():
    """Without an owner header internal callers cannot read user or unowned rows."""
    with _client(INTERNAL_USER) as (client, create_or_reject):
        denied = client.post("/api/runs/stream", json=_body(THREAD_A))
        unowned = client.post("/api/runs/stream", json=_body(THREAD_SHARED))
