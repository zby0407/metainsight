...
class TestThreadMetaRepository:
    ...
    async def test_check_access_no_record_denies(self, repo):
        assert await repo.check_access("unknown", "user1") is False
    ...
    @pytest.mark.anyio
    async def test_check_access_no_owner_denies_all(self, repo):
    ...
    async def test_check_access_strict_null_owner_denied(self, repo):
        """Unowned pre-auth rows must be migrated, never shared at runtime."""
        await repo.create("t1", user_id=None)
