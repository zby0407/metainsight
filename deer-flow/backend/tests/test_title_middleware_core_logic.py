...
class TestTitleMiddlewareCoreLogic:
    ...
    def test_generate_title_local_mode_skips_model_call(self, monkeypatch):
        _set_test_title_config(generation_mode="local", max_chars=20)
        middleware = TitleMiddleware()
        create_model = MagicMock()
        monkeypatch.setattr(title_middleware_module, "create_chat_model", create_model)
        state = {
            "messages": [
                HumanMessage(content="查看我的投资组合"),
                AIMessage(content="正在读取"),
            ]
        }

        result = asyncio.run(middleware._agenerate_title_result(state))

        assert result == {"title": "查看我的投资组合"}
        create_model.assert_not_called()

    def test_structured_workflow_uses_display_label_instead_of_raw_prompt(self, monkeypatch):
        _set_test_title_config(generation_mode="model", max_chars=50)
        middleware = TitleMiddleware()
        create_model = MagicMock()
        monkeypatch.setattr(title_middleware_module, "create_chat_model", create_model)
        state = {
            "messages": [
                HumanMessage(
                    content="请在深研模式下对组合执行完整的后台策略优化指令",
                    additional_kwargs={
        ...
        assert result == {"title": "策略优化 · 长期成长组合"}
        create_model.assert_not_called()
