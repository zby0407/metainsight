...
import { parseNewsContextMessage } from "@/core/finance/news";
import { readPortfolioWorkflowDisplay } from "@/core/finance/workflows";
...
import { MarkdownContent, MarkdownLink } from "./markdown-content";
...
export function MessageListItem({
  ...
  const rawClipboardContent =
    extractContentFromMessage(message) ??
    extractReasoningContentFromMessage(message) ??
    "";
  const portfolioWorkflow = isHuman
    ? readPortfolioWorkflowDisplay(message.content, message.additional_kwargs)
    : null;
  const clipboardContent = isHuman
    ? (portfolioWorkflow?.label ??
      parseNewsContextMessage(rawClipboardContent)?.question ??
      rawClipboardContent)
    : rawClipboardContent;
  return (
    <AIElementMessage
      ...
      {!isLoading && showCopyButton && (
        ...
        >
          <div className="pointer-events-auto flex gap-1">
            <CopyButton clipboardData={clipboardContent} />
...
function MessageContent_({
  ...
  const newsFollowUp = useMemo(
    () => (isHuman ? parseNewsContextMessage(rawContent) : null),
    [isHuman, rawContent],
  );
  const portfolioWorkflow = useMemo(
    () =>
      isHuman
        ...
        : null,
    [isHuman, message.additional_kwargs, message.content],
  );
  ...
  const contentToDisplay = useMemo(() => {
    if (isHuman) {
      if (portfolioWorkflow) return portfolioWorkflow.label;
      if (newsFollowUp) return newsFollowUp.question;
      return rawContent ? stripUploadedFilesTag(rawContent) : "";
    }
    return rawContent ?? "";
  }, [rawContent, isHuman, newsFollowUp, portfolioWorkflow]);
    ...
    return (
      <div
        className={cn(
          ...
          newsFollowUp ? "w-full max-w-xl" : className,
        )}
      >
        {filesList}
        {newsFollowUp && <NewsChatContextCard context={newsFollowUp.context} />}
        {contentToDisplay && (
          <AIElementMessageContent
            className={cn(
              "max-w-full",
              newsFollowUp ? "ml-auto w-fit" : "w-full",
            )}
          >
            <div
              className="break-words whitespace-pre-wrap"
              data-testid={
                portfolioWorkflow ? "portfolio-workflow-message" : undefined
              }
            >
              {contentToDisplay}
            </div>
