...
test.describe("Thread list infinite scroll (issue #3482)", () => {
  ...
  test("conversation history panel loads more threads when scrolling to the bottom", async ({
    page,
  }) => {
    mockLangGraphAPI(page, { threads: THREADS });
    ...
    await page.getByRole("button", { name: /对话记录|Conversation history/ }).click();

    // The 50th thread (end of first page) appears in the conversation panel.
