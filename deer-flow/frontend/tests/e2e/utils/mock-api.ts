...
export type MockAPIOptions = {
  ...
  agents?: MockAgent[];
  skills?: MockSkill[];
  portfolioDashboard?: unknown;
  portfolioDashboardAfterSetup?: unknown;
  portfolioSetupResponse?: unknown;
  onPortfolioSetup?: (payload: unknown) => void;
  workflowAgentResult?: string;
  onWorkflowRun?: (payload: unknown) => void;
  onChatRun?: (payload: unknown) => void;
};
...
export function mockLangGraphAPI(page: Page, options?: MockAPIOptions) {
  ...
  let portfolioDashboard = options?.portfolioDashboard ?? {
    summary: {
      portfolioCount: 0,
      activeCount: 0,
      withStrategyCount: 0,
  ...
  void page.route("**/api/finance/portfolio-dashboard", (route) => {
    if (route.request().method() === "GET") {
      return route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify(portfolioDashboard),
      });
    }
    return route.fallback();
  });

  void page.route("**/api/finance/portfolio-setup", (route) => {
    if (route.request().method() === "POST") {
      const payload = route.request().postDataJSON() as unknown;
      options?.onPortfolioSetup?.(payload);
      if (options?.portfolioDashboardAfterSetup) {
        portfolioDashboard = options.portfolioDashboardAfterSetup;
      }
      return route.fulfill({
        status: 201,
        contentType: "application/json",
        body: JSON.stringify(
          options?.portfolioSetupResponse ?? {
            ...
            account: {
              portfolioId: "51eea9b8-f3c2-4e48-a28c-7b51c01792bf",
              portfolioRevision: 2,
              baseCurrency: "CNY",
              positions: [],
              cashBalances: [],
  ...
  const handleMockRunStream = (route: Route) => {
    options?.onChatRun?.(route.request().postDataJSON() as unknown);
    upsertThread({
      thread_id: MOCK_THREAD_ID,
      title: "New Chat",
      updated_at: new Date().toISOString(),
  ...
  void page.route("**/api/langgraph/runs/wait", (route) => {
    if (route.request().method() === "POST") {
      const payload = route.request().postDataJSON() as unknown;
      options?.onWorkflowRun?.(payload);
      return route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({
