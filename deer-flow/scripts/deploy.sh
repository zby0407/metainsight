...
# The portfolio Agent bridge is opt-in. When its secret is present in the
# deployment environment or root .env, attach only the Gateway to the external
# finance service network. The public frontend never joins that network.
finance_bridge_configured=false
if [ -n "${DEERFLOW_FINANCE_BRIDGE_SECRET:-}" ]; then
    finance_bridge_configured=true
elif [ -f "$REPO_ROOT/.env" ] && grep -Eq '^DEERFLOW_FINANCE_BRIDGE_SECRET=.+$' "$REPO_ROOT/.env"; then
    finance_bridge_configured=true
fi

if [ "$finance_bridge_configured" = "true" ]; then
    finance_network="${DEERFLOW_FINANCE_NETWORK:-pipecat_default}"
    if ! docker network inspect "$finance_network" >/dev/null 2>&1; then
        echo -e "${RED}✗ Finance bridge network does not exist: $finance_network${NC}" >&2
        exit 1
    fi
    COMPOSE_CMD+=(-f "$DOCKER_DIR/docker-compose.finance.yaml")
    echo -e "${GREEN}✓ Finance Agent bridge network: $finance_network${NC}"
fi
