...
async def _ensure_admin_user(app: FastAPI) -> None:
    ...
    "no-auth → with-auth" upgrade path: users who ran the earlier version without
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    async with langgraph_runtime(app, startup_config):
        ...
        dsa_automation_task = asyncio.create_task(
            ...
            name="zhiheng-dsa-automation",
        )

        try:
            yield
        finally:
            dsa_automation_task.cancel()
            try:
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="知衡 API Gateway",
        description="""
## 知衡 API Gateway

知衡智能研究工作台的 API 网关，提供智能体编排与沙箱执行能力。
...
This gateway provides runtime endpoints for agent runs plus custom endpoints for models, MCP configuration, skills, and artifacts.
    ...
    # Account-scoped post-close DSA automation and notification inbox.
    ...
    async def health_check() -> dict[str, str]:
        ...
        """
        return {"status": "healthy", "service": "zhiheng-gateway"}
