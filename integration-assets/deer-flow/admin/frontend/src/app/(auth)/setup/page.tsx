...
export default function SetupPage() {
  ...
  useEffect(() => {
    if (mode === "change_password" && user?.email) {
      setEmail(user.email);
    }
  }, [mode, user?.email]);
  ...
  if (mode === "init_admin") {
    return (
      <div className="bg-background flex min-h-screen items-center justify-center">
        <FlickeringGrid
          className="absolute inset-0 z-0 mask-[url(/images/zhiheng-mask.svg)] mask-size-[100vw] mask-center mask-no-repeat md:mask-size-[72vh]"
          squareSize={4}
          gridGap={4}
        ...
        <div className="border-border/20 bg-background/5 w-full max-w-md space-y-6 rounded-3xl border p-8 backdrop-blur-sm">
          <div className="text-center">
            <h1 className="font-serif text-3xl">知衡</h1>
  ...
  return (
    <div className="bg-background flex min-h-screen items-center justify-center">
      ...
      <div className="border-border/20 bg-background/5 w-full max-w-md space-y-6 rounded-3xl border p-8 backdrop-blur-sm">
        <div className="text-center">
          <h1 className="font-serif text-3xl">知衡</h1>
          <p className="text-muted-foreground mt-2">完成账号设置</p>
          <p className="text-muted-foreground mt-1 text-xs">
            首次登录需要确认邮箱并设置你自己的密码。
          </p>
        </div>
