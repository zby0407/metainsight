...
const NAV_ITEMS: NavItem[] = [
  ...
  { key: 'chat', labelKey: 'layout.nav.chat', to: '/login?next=%2Fworkspace%2Fchats%2Fnew', icon: MessageSquareQuote, external: true },
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <nav className={cn('flex flex-col gap-1.5', isRail ? '' : 'flex-1')} aria-label={t('layout.mainNav')}>
        {navItems.map(({ key, labelKey, to, icon: Icon, exact, badge, external }) => {
          ...
          if (external) {
            return (
              ...
              >
                <Icon className={cn(itemIconClass, 'text-current')} />
