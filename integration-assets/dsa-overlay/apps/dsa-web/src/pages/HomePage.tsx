...
const HomePage: React.FC = () => {
  ...
  const handleAskFollowUp = useCallback(() => {
    ...
    const researchPath = `/workspace/chats/new?stock=${encodeURIComponent(code)}&name=${encodeURIComponent(name)}&recordId=${rid}`;
    window.location.assign(`/login?next=${encodeURIComponent(researchPath)}`);
  }, [selectedReport]);
        ...
        <header className="relative z-30 flex min-w-0 flex-shrink-0 items-center overflow-visible px-3 py-3 md:px-4 md:py-4">
          <div className="flex min-w-0 flex-1 flex-col gap-2.5 md:flex-row md:items-center">
            ...
            <div className="flex min-w-0 flex-shrink-0 items-center gap-2.5">
              ...
              <a
                href="/login?next=%2Fworkspace%2Fchats%2Fnew%3Fmarket%3Dcn"
                className="home-surface-button flex h-10 flex-1 items-center justify-center gap-1.5 whitespace-nowrap rounded-xl px-3 text-xs font-medium text-primary md:flex-none"
              >
